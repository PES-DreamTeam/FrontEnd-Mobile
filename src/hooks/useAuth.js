...
const useAuth = () => {
    ...
    const loginWithSocialMedia = async (token, socialMedia) => {
        return await axios.post(`${API_HOST}/api/auth/social-login?token=${token}`, {
            socialMedia
        });
    }
    ...
    return { signIn, signOut, signUp, isSignedIn, updateUser, deleteAccount, loginWithFacebook, setAuth, auth };
}
